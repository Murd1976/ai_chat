...
# папка базы данных
persist_directory = '/content/drive/MyDrive/ColabNotebooks/ChatGPT/DB/CrisEmbeddingsChat/'
# путь к учебным материалам
data_directory = '/content/drive/MyDrive/ColabNotebooks/ChatGPT/DB/Cris/'

os.environ["OPENAI_API_KEY"] = "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"
openai.api_key = "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"
...
class WorkerОpenAI():
  ...
  def create_embedding(self, doc_dir="", persist_directory=""):
    ...
    db_faiss = (FAISS.from_documents(self.source_chunks[0], OpenAIEmbeddings()))
    for i in range(len(self.source_chunks) - 1):
        db_faiss.add_documents(FAISS.from_documents(self.source_chunks[i], OpenAIEmbeddings()))
    db_faiss.as_retriever()

    # также можно сохранить хранилище локально
    db_faiss.save_local('/content/drive/MyDrive/ColabNotebooks/ChatGPT/DB/faiss_index')
  ...
  def answer_index(self, system, topic, search_index, temp=1, verbose=0):
      ...
      if verbose: 
        ...
        self.debug_log.append('\n ===========================================: ')
        self.debug_log.append(f"\n {self.num_tokens_from_messages(messages, 'gpt-3.5-turbo-0301')} tokens used for the question")
